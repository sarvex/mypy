...
class ExpressionChecker(ExpressionVisitor[Type]):
    ...
    def module_type(self, node: MypyFile) -> Instance:
        ...
        for name, n in node.names.items():
            ...
            if isinstance(n.node, Var) and n.node.is_final:
                immutable.add(name)
            if typ := self.chk.determine_type_of_member(n):
    ...
    def check_typeddict_call_with_kwargs(
        ...
        elif callee.fallback.type.special_alias is None:
    ...
    def check_callable_call(
        ...
        if callee.is_type_obj():
            if callee.type_object().is_protocol and not callee.from_type_type:
                self.chk.fail(
                ...
                abstract_attributes: dict[str, bool] = {
                    attr_name: self.can_return_none(type, attr_name)
    ...
    def check_for_extra_actual_arguments(
        ...
        for i, kind in enumerate(actual_kinds):
            if (
                ...
                ) and all_actuals.get(i, 0) < len(actual_type.items):
    ...
    def visit_op_expr(self, e: OpExpr) -> Type:
        ...
        if e.op not in operators.op_methods:
